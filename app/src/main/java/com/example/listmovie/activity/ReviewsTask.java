...
import static com.example.listmovie.data.Api.ApiKey;
...
public class ReviewsTask extends AsyncTask<Long, Void, List<Reviews>> {
    ...
    protected List<Reviews> doInBackground(Long... params) {
        ...
        try {
            ...
            URL url;
            url = new URL("http://api.themoviedb.org/3/movie/"+movieId +"/reviews?api_key="+ApiKey);
            ...
            Gson gson = new Gson();
            String results = gson.toJson(inputStream);  //IOUtils to convert inputstream objects into Strings type
